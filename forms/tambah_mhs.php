<?php 
    ...
    $kelas = queryReadAll("SELECT * FROM kelas WHERE nip = '$loggednip'"); // get all kelas
    $mahasiswa = queryReadAll("SELECT * FROM mahasiswa"); // get all mahasiswa
    $rowcount = getRowCount("SELECT * FROM kelas_mahasiswa WHERE nip = '{$_SESSION["nip"]}'");

    if(isset($_POST['submit'])) {
        if(queryEnroll($_POST) > 0) {
            echo 
                '<script> 
                alert("Sukses")
                </script>
...
<body class="bg-slate-50">
    ...
    <div class="py-12 px-24 sm:ml-64">
        ...
        <form action="" method="post" class="p-4">
            <div class="mb-6">
                <label for="idkelas" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama Kelas</label>
                <select type="text" id="idkelas" name="idkelas" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required>
                    <?php foreach ($kelas as $kls) : ?>
                    <option class="text-base" value="<?= $kls['id_kelas'] ?>"><?= $kls['id_kelas'] ?> - <?= $kls['nama_kelas'] ?></option>
                    <?php endforeach; ?>
                </select>
            </div>

            <div class="mb-6">
                <label for="nrp" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Mahasiswa</label>
                <select type="text" id="nrp" name="nrp" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required>
                    <?php foreach ($mahasiswa as $mhs) : ?>
                    <option class="text-base" value="<?= $mhs['nrp'] ?>"><?= $mhs['nrp'] ?> - <?= $mhs['nama'] ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="mb-6">
                <label for="nilai" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nilai</label>
                <input type="text" id="nilai" name="nilai" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required>
            </div>
            <div class="mb-6">
                <label for="nip" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white"></label>
                <input type="hidden" id="nip" name="nip" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" value="<?= $loggednip ?>" required>
            </div>
