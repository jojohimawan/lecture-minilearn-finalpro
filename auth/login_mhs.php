    ...
    if(isset($_POST['loginmhs'])) {
        ...
        if(mysqli_num_rows($result) === 1) { // if found
            ...
            if(password_verify($password, $row['password'])) { // verify pass
                ...
                $_SESSION['usernamemhs'] = $username;
                $_SESSION['nrp'] = $row['nrp'];

                // redirect if pass verified
                header("Location: ./../homepages/home_mhs.php"); 
                exit;
            }
        }
    ...
    <div class="grid grid-cols-2 gap-5 h-screen">
        <div class="w-full p-40 flex flex-col gap-y-12">
            <div class="flex flex-col gap-y-4">
                ...
                <p class="text-3xl font-bold">Login Mahasiswa</p>
                <div class="">
                    <a href="./register_mhs.php" class="text-lg text-slate-500">No account?</a>
                    <a href="./register_mhs.php" class="text-lg text-green-700 font-medium">Register</a>
                </div>
                <?php if( isset($error) ) : ?>
            ...
            <form action="" method="post">
                ...
                <button type="submit" class="text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-green-300 font-medium rounded-lg text-sm w-full sm:w-full px-5 py-2.5 text-center" name="loginmhs">Login</button>
            </form>
